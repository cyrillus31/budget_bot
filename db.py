...
async def insert_expenses(amount, category=""):
    ...
    query = "INSERT INTO expenses VALUES(?, ?, datetime(), strftime('%m', 'now'))"
...
async def insert_monthly_limit(amount):
    ...
    query = "INSERT INTO limits VALUES(strftime('%m', 'now'), ?)"
...
async def update_monthly_limit(amount):
    ...
    query = "UPDATE limits SET monthly_limit=? WHERE month=strftime('%m', 'now')"
    cursor.execute(query, (amount,))
    conn.commit()
...
async def get_monthly_limit() -> str:
    ...
    query = "SELECT monthly_limit FROM limits WHERE month=strftime('%m', 'now')"
...
async def get_todays_expenses() -> str:
    ...
    query = """SELECT SUM(amount) FROM expenses 
                WHERE strftime('%m-%d', time) AND category!='0' 
                AND category!=0"""
    res = cursor.execute(query)
    return str(res.fetchone()[0])
...
async def get_this_month_expenses() -> str:
    "Returns how much money was spent today"
    query = """SELECT SUM(amount) FROM expenses 
                WHERE limit_id=strftime('%m', 'now') AND category!='0' 
                AND category!=0"""
    res = cursor.execute(query)
    return str(res.fetchone()[0])
